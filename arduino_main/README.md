...
To make the encoder work properly, we have to keep the run time for one loop under 35ms. Since this is nearly impossible if we update the TOF distance sensor on every loop, we decide to update the encoder serveral times in the loop
